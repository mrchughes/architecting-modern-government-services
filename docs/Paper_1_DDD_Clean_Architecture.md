...
| 1.3 | Mar 2026 | Rewrote §1.1 to clarify recursive subdomain decomposition; added two-level decomposition (product lines to functional subdomains); clarified Core/Supporting/Generic classification with consistent examples |
| 1.4 | Mar 2026 | Added manufacturing analogy introduction to Part I: Ford/assembly line parallel to domain decomposition |
| 1.5 | Mar 2026 | Added §1.5 "Using External Data for Decisions": three patterns for cross-context data; anti-corruption layer; Identity to Eligibility to Payments flow diagram |
...
- **Simple subdomains** often map 1:1 to bounded contexts (Identity subdomain to one Identity bounded context)
- **Complex subdomains** may need multiple bounded contexts (Benefits subdomain to separate Eligibility, Claims, and Awards contexts if the language diverges enough)
...
| **Clean Architecture** | Code layers within a microservice | (see Part III) | Entities, Use Cases, Adapters |
...
flowchart TB
    subgraph Platform["Platform Services"]
        Identity["Identity Platform (Open Host Service)"]
        CX360["Customer360 (Shared Kernel)"]
        Payments["Payments (Customer-Supplier)"]
        Notif["Notifications (Open Host Service)"]
    end
...
Import direction: outer to inner. Rule satisfied.
...
- Use case imports `RefundRepository` interface, but that interface lives *in its own layer*. Inner to inner.
- `DynamoRefundRepository` imports `RefundRepository` to implement it. Outer to inner.
...
|-----------|--------------|---------------------|-----|
| Controller to UseCase | Outer calls inner | Outer imports inner | Just import |
| UseCase to Database | Inner calls outer | Inner imports... inner | Define interface in inner layer, outer implements it |
...
It knows nothing about DynamoDB, SNS, or HTTP. It orchestrates domain entities and calls interface methods. **Dependency direction: use case depends on interfaces (contracts owned by use case); infrastructure implements them.**
...
The **dependency direction is always inward**: infrastructure depends on use case interfaces, controller depends on use case, use case depends on domain entities. The core (domain and use cases) never knows about the outer layers (infrastructure and frameworks).
...
```mermaid
flowchart TB
    E2E["E2E Journey Tests (critical paths only, slow, brittle)"]
    Domain["Domain Rule Tests (BDD specs, executable invariants)"]
    Contract["Contract Tests (Provider + Consumer, run on every commit)"]
    Unit["Unit + Integration Tests (bulk of testing, fast, focused)"]
